...
ENV TEZ_VERSION tez-0.8.4.2.6.0.3-8
...
RUN curl -s http://s3.amazonaws.com/public-repo-1.hortonworks.com/HDP/centos6/2.x/updates/2.6.0.3/tars/tez_hive2/$TEZ_VERSION.tar.gz > /tmp/$TEZ_VERSION.tar.gz
RUN tar -C /usr/local/$TEZ_VERSION -xzf /tmp/$TEZ_VERSION.tar.gz

RUN cd /usr/local && ln -s $TEZ_VERSION tez
RUN mkdir -p /usr/local/tez/share && curl -s http://s3.amazonaws.com/public-repo-1.hortonworks.com/HDP/centos6/2.x/updates/2.6.0.3/tars/tez_hive2/$TEZ_VERSION-minimal.tar.gz > /usr/local/tez/share/tez.tar.gz
